...
def multi_Saab_transform(images, labels, kernel_sizes, num_kernels, energy_percent, use_num_images, use_classes, stride):
    '''
    Do the PCA "training".
    :param images: [num_images, height, width, channel]//maybe not correct
...
def initialize(sample_images, pca_params, pre_stage=0,stride=4):
    ...
    for i in range(num_layers):
        ...
        # Maxpooling
        # sample_images = block_reduce(sample_images, (1, 2, 2, 1), np.max)
        #########sample_images = block_reduce(sample_images, (1, 1, 2, 2), np.max, cval=-10000)
